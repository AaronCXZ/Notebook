...
- 再子类中新建一个字段，使其引用超类的一个对象，并就这个委托引用初始化为超类的新实例
- 针对超类的每个函数，在子类中创建一个转发函数，就调用请求转发给委托引用。每转发一块完整逻辑，都要执行测试
- 当所有超类都被转发函数覆写后，就可以去掉继承关系
...
class Scroll extends CatalogItem {
    ...
    daysSinceLastCleaning(targetDate){
        return this._lastCleaned.until(targetDate, shronoUtil.DAYS);
    }
}
```
